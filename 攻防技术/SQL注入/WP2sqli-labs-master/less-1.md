...
## 判断是否存在SQL注入
*根据提示，发现是GET传参方式，参数是id*
![示意图1-1](./images/less-1-001.png)

*尝试SQL注入，发现存在_字符型注入_，单引号（'）闭合方式，有回显。可以用联合注入*
```python  
?id=1
#完整sql查询语句如下
...
## 联合注入
*首先得知道当前查询语句返回的表格有多少列,_union前后拼接的查询语句列数必须相同_。*
...
*联合注入，判断回显位置。发现回显位是2，3*
```python
?id=-1' union select 1, 2, 3 --+ 
```
![示意图1-4](./images/less-1-004.png)

*查询数据库类型、版本和当前数据库名，是mysql数据库，版本5.5.53，数据库名security*
```python
